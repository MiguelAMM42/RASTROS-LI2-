...
void jogadaAnterior (ESTADO *e, int numeroJogada) { // Supõe-se que se recebe um *numeroJogada* válido, ou seja, menor que o num_jogadas atual.
    ...
    numeroJogada ++;
    while (n != e -> num_jogadas) { // Transformar as peças das eventuais jogadas posteriores.
        set_casa (e, e -> jogadas[n].jogador1, VAZIO);
        set_casa (e, e -> jogadas[n].jogador2, VAZIO);
        numeroJogada ++;
    }
    if (get_jogador_atual (e) == 2) set_casa (e, e -> jogadas[n].jogador1, VAZIO); // Caso a última jogada seja incompleta.
