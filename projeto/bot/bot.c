...
int test_max (MAPA m1, MAPA m2, int res, COORDENADA cAtual, ESTADO *e, int * val, int pos)
{
        if (CoordenadaValida (&cAtual)) {
                                   int valTab1, valTab2, temp;
                                    valTab1 = m1[cAtual.linha][cAtual.coluna];
                                    valTab2 = m2[cAtual.linha][cAtual.coluna];
                                 ...
                                 if (valTab1 == 100 && valTab2 == 100) temp = paridade (e);
                            //     if (valTab1 == 100) temp = valTab2;
                             //    if (valTab2 == 100) temp = valTab1;
...
int test_min (MAPA m1, MAPA m2, int res, COORDENADA cAtual, ESTADO *e, int * val, int pos)
{
   if (CoordenadaValida (&cAtual)) {
                                 int valTab1, valTab2, temp;
                                    valTab1 = m1[cAtual.linha][cAtual.coluna];
...
//mostrar_mapa ( m2) ;
    ...
    if (min_max == 1) {  //Para maximizar
        ...
        if ((coorJ.coluna <= maximino && valTab1 != -1 && valTab2 != -1  && valTab1 == 100 && valTab2 == 100)) res = valTab2  -  valTab1 ;
        else res = -300;
                                        //Inicializar valor de res
        valores[posArrays] = res;
        valIguais[posArrays] = m2[coorJ.linha][coorJ.coluna];
        posArrays++;
        (coorJ.linha)++;

        res = test_max (m1, m2, res, coorJ, e, valores, posArrays);
        valIguais[posArrays] = m2[coorJ.linha][coorJ.coluna];
        posArrays++;
        (coorJ.coluna)--;
...
//        posArrays++;
        int casasIguais = 0;
        int i;
        for (i = 0; i <= 7; i++)  if (valores[i] = res) casasIguais++;
        res = -14;
        int temp;
        i= 7;
        if (casasIguais >= 2) while (i >= 0) {
                                            temp = valIguais[i];
...
//Inicializar valor de resposta
        ...
        (coorJ.linha)++;
        res = test_min (m1, m2, res, coorJ, e, valores, posArrays);
        ...
        for (i = 0; i <= maximino; i++)
            {
                for (ii = 0; ii<= maximino; ii++)
                {
                   ...
                   if (v1 == -1 || v2 == -1 || (v1 == 100 && v2 == 100)) v2 = -69;
                   else v2 = v2-v1;
...
int minimax(ESTADO *e, int comp, int alpha, int beta, int Maximizar) { 
  ...
  //  if (jogAtual == 1) jogAtual = 2;
    ...
    else if (!comp) return min_max_Estado (e, jogAtual ); // min_max_Estado(&e, Maximizar);
